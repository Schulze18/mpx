...
def split_constraint_torques(model, data):
    mujoco.mj_forward(model, data)

    nefc, nv = data.nefc, model.nv
    efc_force = np.array(data.efc_force[:nefc], dtype=float)
    efc_type  = np.array(data.efc_type[:nefc], dtype=int)
    ...
    mask_contact  = np.isin(efc_type, contact_types)
    mask_limit    = (efc_type == mujoco.mjtConstraint.mjCNSTR_LIMIT_JOINT)
    mask_fric_dof = (efc_type == mujoco.mjtConstraint.mjCNSTR_FRICTION_DOF)

    e_contact  = np.where(mask_contact,  efc_force, 0.0)
    e_limit    = np.where(mask_limit,    efc_force, 0.0)
    e_fricdof  = np.where(mask_fric_dof, efc_force, 0.0)

    tau_contact = np.zeros(nv); mujoco.mj_mulJacTVec(model, data, tau_contact, e_contact)
    tau_limit   = np.zeros(nv); mujoco.mj_mulJacTVec(model, data, tau_limit,   e_limit)
    tau_fricdof = np.zeros(nv); mujoco.mj_mulJacTVec(model, data, tau_fricdof, e_fricdof)

    return tau_contact, tau_limit, tau_fricdof
...
def add_sim_data_to_dataset(dataset, time, sim_mj_data, sim_mj_model, feet_geom_id, feet_body_id, sim_mj_data_aux = None):
    ...
    # print(f"Contact state: {contact_state} total feet contact forces: {total_feet_contact_forces} full wrench contact: {full_wrench_contact}")
    ...
    # tau_component_contact, tau_component_limit, tau_component_fricdof = split_constraint_torques(sim_mj_model, sim_mj_data)

    # print(f'\ntau_component_contact : {tau_component_contact}')
    # print(f'tau_component_limit     : {tau_component_limit}')
    # print(f'tau_component_fricdof   : {tau_component_fricdof}')
    ...
    # print(f'qfrc_constraint        : {sim_mj_data.qfrc_constraint}')
    # print(f'full_wrench_contact    : {full_wrench_contact}')
    ...
    #         # print(
    #         #     f"constraint {item['index']:3d} | {item['type_name']:<22s} | id {item['id']:3d} | force {item['force']:+.6f} | tau_norm {tau_norm:.6f} | tau {np.array2string(item['tau'], precision=3, suppress_small=True)}"
    #         # )
    #         print(
    #             f"constraint {item['index']:3d} name {item['type_name']} | tau {np.array2string(item['tau'], precision=3, suppress_small=True)}"
    #         )
...
def main():
    ...
    # print(f"contact_ids {contact_ids} Feet geom IDs: {feet_geom_id} feet body IDs: {feet_body_id}")
